...
Aryan must expose a Prisma client with these agreed models and fields:
...
Aryan's JWT middleware must set `req.user` to `{ id: UUID, role: VENDOR | BUYER | ADMIN }` before the Person 3 routers. This module deliberately derives vendor and reviewer IDs from that identity, never request-body IDs.
